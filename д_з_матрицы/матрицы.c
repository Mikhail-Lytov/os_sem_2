...
void* matrix_multiplication_1(void *argv){
	for (i = 0; i < (line/2); i++){
        for(j = 0; j < column; j++){
            for (k = 0; k < line; k++){
...
void* matrix_multiplication_2(void *argv){
...
void* start(void *argv){
	...
	pthread_create(&mat_one, NULL, matrix_multiplication_1, NULL);		
	pthread_join(mat_one, NULL);

	pthread_create(&mat_two, NULL, matrix_multiplication_2, NULL);		
	pthread_join(mat_two, NULL);
}
int main(int argc,char* argv[]) 
{
    ...
    int status;
    int status_addr;
    status = pthread_create(&thread, NULL, start, NULL);
